...
// TODO: move this somewhere client can also use
const LoaderDatabaseVersion = 1;
const LoaderDatabaseName = 'GenLiteLoaderDatabase';
const GenfanadStoreName = 'genfanad.client';
const GenliteStoreName = 'genlite.client';

function setupIDB() {
    let r = window.indexedDB.open(LoaderDatabaseName, LoaderDatabaseVersion);
    r.onupgradeneeded = (e) => {
        let db = r.result;

        if (!db.objectStoreNames.contains(GenfanadStoreName)) {
            let store = db.createObjectStore(GenfanadStoreName, {
                keyPath: 'key',
                autoIncrement: true
            });
        ...
        if (!db.objectStoreNames.contains(GenliteStoreName)) {
            let store = db.createObjectStore(GenliteStoreName, {
                keyPath: 'key',
                autoIncrement: true
            });
            store.createIndex('indexKey', 'key', {unique: true});
    ...
    r.onerror = (e) => {
        console.error('Error initializing Genlite IDB', e);
    };
}

function getGenliteFromIDB(callback) {
    let r = window.indexedDB.open(LoaderDatabaseName, LoaderDatabaseVersion);
    r.onsuccess = (e) => {
        let db = r.result;
        let tx = db.transaction(GenliteStoreName, 'readonly');
        let store = tx.objectStore(GenliteStoreName);
...
function saveGenliteToIDB(src, version, timestamp, callback) {
    let r = window.indexedDB.open(LoaderDatabaseName, LoaderDatabaseVersion);
    r.onsuccess = (e) => {
        let db = r.result;
        let tx = db.transaction(GenliteStoreName, 'readwrite');
        let store = tx.objectStore(GenliteStoreName);

        // clear first, so we only have one at a time
        store.clear().onsuccess = (e) => {
            store.put({
                src: src,
                version: version,
                timestamp: timestamp,
            }).onsuccess = (e) => { callback(); };
        };
    };
}

function getGenfanadFromIDB(callback) {
    let r = window.indexedDB.open(LoaderDatabaseName, LoaderDatabaseVersion);
    r.onsuccess = (e) => {
        let db = r.result;
        let tx = db.transaction(GenfanadStoreName, 'readonly');
        let store = tx.objectStore(GenfanadStoreName);
...
function saveGenfanadToIDB(src, timestamp) {
    let r = window.indexedDB.open(LoaderDatabaseName, LoaderDatabaseVersion);
    r.onsuccess = (e) => {
        let db = r.result;
        let tx = db.transaction(GenfanadStoreName, 'readwrite');
        let store = tx.objectStore(GenfanadStoreName);
...
function setupGenlite() {
    getGenliteFromIDB((genlite) => {
        ...
        let needsUpdate = genlite === null;
        if (!needsUpdate) {
            // if we have a cached version, check if it's old
            let genliteUpdateTimestampDate = new Date(genlite.timestamp);
            needsUpdate = genliteLastModified > genliteUpdateTimestampDate;
        }
        ...
        if (needsUpdate) {
            ...
            window.addEventListener('load', function () {
                ...
                version.innerText = 'GenLite Version:' + genlite.version;
...
function setupGenfanad() {
    getGenfanadFromIDB((genfanad) => {
        ...
        let genfanadJS = genfanad?.src;
        if (genfanad === null || genfanadLastModified > new Date(genfanad.timestamp)) {
            ...
            xhrClientJS.onload = function () {
                if (xhrClientJS.status == 200) {
                    ...
                    saveGenfanadToIDB(genfanadJS, genfanadLastModified.toString());
                } else {
...
if ('indexedDB' in window) {
    ...
    console.error('GenLite is not supported in this browser (no IndexedDB)');
}
